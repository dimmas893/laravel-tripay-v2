        ...
        < Kembali </a>
            ...
            <div class="grid grid-cols-5 gap-3 mt-6">
                <div class="col-span-4">
                    <div class="grid grid-cols-4 gap-3">
                        @foreach ($channels->data as $channel)
                            @if ($channel->active)
                                <form action="{{ route('transaction.store', $book->id) }}" method="POST"
                                    class="cursor-pointer">
                                    @csrf
                                    <input type="hidden" name="book_id" value="{{ $book->id }}">
                                    <input type="hidden" name="method" value="{{ $channel->code }}">
                                    <button type="submit"
                                        class="bg-white p-5 h-32 w-36 rounded-md shadow-soft flex items-center">
                                        <div>
                                            <img src="{{ $channel->icon_url }}" class="w-full h-10 object-contain"
                                                alt="">
                ...
                <div class="col-span-1">
                    <img class="object-contain rounded-md" src="{{ asset('storage/' . $book->cover_image) }}"
                        alt="">
